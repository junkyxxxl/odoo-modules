...
Source0:    src.tar.gz
Source1:    odoo-server.conf
Source3:    odoo-gen-cert
...
install -d %{buildroot}/%{_datadir}/odoo/pixmaps

install -d %{buildroot}%{_localstatedir}/spool/odoo
install -d %{buildroot}%{_localstatedir}/run/odoo

install -d %{buildroot}/var/log/odoo

# Install the init scripts and conf
install -m 644 -D %SOURCE1 %{buildroot}%{_sysconfdir}/odoo/odoo-server.conf
...
%pre
getent group odoo >/dev/null || groupadd -r odoo
getent passwd odoo >/dev/null || \
    useradd -r -g odoo -d /home/odoo -s /sbin/nologin \
    -c "Odoo Server" odoo
#usermod -d /home/odoo odoo >/dev/null 2>&1 || :
...
pip install ofxparse --upgrade
pip install flanker --upgrade
...
%attr(0755,odoo,odoo) %{_localstatedir}/run/odoo
%attr(0755,root,odoo) %dir %{_sysconfdir}/odoo
%attr(0755,odoo,odoo) %dir /var/log/odoo
%attr(0660,root,odoo) %config(noreplace) %{_sysconfdir}/odoo/odoo-server.conf
#%config(noreplace) %{_sysconfdir}/logrotate.d/openerp-server
%{_datadir}/odoo/pixmaps/

%changelog

* Mon Jun 22 2015 Andrea Stirpe <a.stirpe@isa.it> 9.0-1
  - First RPM package for Odoo 9.0.
