...
class CsvExportOpenERPInterface(report.interface.report_int):
    ...
    def create(self, cr, uid, ids, data, context):
        ...
        if query_type != 'sql':
            raise orm.except_orm(('Error !'), ("Not supported yet! Use SQL instead"))
        ...
        if '%s' in config_query and 'ea_export.config' not in [context.get('active_model'), context.get('filter_model')] and active_ids:
            ...
            raise orm.except_orm(('Error !'), ("You can execute query with '%s' formatting from {model} model only!".format(model=config_value.get('related_model'))))
        else:
            query = config_query
        ...
        try:
            ...
            raise orm.except_orm(('Error !'), ("Invalid Query"))
