...
%%	A literal '%' character.),"""),
    ...
    def _match_limit(self, template_line, result):
            ...
            if not all([bool(field_str) == False for field_str in input_list]):
                ...
                if template_line_type == 'binary':
                    ...
                    if not target_string:
                        raise orm.except_orm(('Error !'), ("Field %s has no value and cannot be converted.  Enter a default value in the template for this field or fix the CSV file so it contains data for this field row. \nField: %s\nColumn: %s\nRow number: %i \nRow Data: %s") % (template_line.target_field.name, template_line.target_field.name, str(template_line.sequence), row_number+1, input_list))
                    ...
                    if template_line.many2one_rel_type in ['dbid', 'active_id']:
                        ...
                        result = self._match_limit(template_line, result)
                        if testing:
