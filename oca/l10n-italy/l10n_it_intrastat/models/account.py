...
class account_invoice_line(models.Model):
    ...
    def _get_price_after_discount(self):
        if self.invoice_id.global_discount_lines:
            tmp = self.price_subtotal            
            for discount in self.invoice_id.global_discount_lines:
                tmp = tmp * (1 - discount.value/100.0)            
            return tmp
        else:
    ...
    def _prepare_intrastat_line(self):
        ...
        # Amount
        amount_currency = self._get_price_after_discount()
        ...
        if 'intrastat_code_id' in intrastat_data and intrastat_data['intrastat_code_id'] and self.env['report.intrastat.code'].browse(intrastat_data['intrastat_code_id']).additional_unit_required:
            res.update({'additional_units': self.quantity})
        else:
            res.update({'additional_units': 0})
        # Transport
        ...
        # Supply method
        supply_method = 'R'
        res.update({'supply_method':supply_method})
        # Payment method
        payment_method = False
        ...
        res.update({'invoice_number': invoice_number})
        #Invoice Date
        invoice_date = self.invoice_id.date_invoice
        res.update({'invoice_date': invoice_date})
...
class account_invoice(models.Model):
    ...
    def action_move_create(self):
        ...
        for invoice in self:
            if invoice.intrastat:
                ...
                if abs(total_amount - invoice.amount_untaxed) > 0.0001:
    ...
    def write(self,vals):
        ...
        if 'inner_intrastat' not in self._context or not self._context['inner_intrastat']:
            self.compute_intrastat_lines()
        return res
    ...
    def compute_intrastat_lines(self):
        ...
        for line in self.invoice_line:
            # Lines to compute
            if not line.product_id or line.free:
                continue
        ...
        if intrastat_lines:
            self.with_context(inner_intrastat=True).write({'intrastat_line_ids' : intrastat_lines})
