...
class WizardExportFatturapa(orm.TransientModel):
    ...
    def setDatiGeneraliDocumento(self, cr, uid, invoice, body, context=None):
        ...
        body.DatiGenerali.DatiGeneraliDocumento.ImportoTotaleDocumento = '%.2f' % invoice.amount_total
        ...
        if invoice.payment_term:
            ...
            if tax_payability == 'S':
                ...
                for move_line_id in payment_line_ids:
                    move_line = move_line_pool.browse(cr, uid, move_line_id, context=context)         
                    
                    if move_line.date_maturity: 
                        mat_date = move_line.date_maturity
                        
                    if invoice.type == 'out_refund':
                        paym_amount = paym_amount + move_line.credit - move_line.debit
                    else:
                        paym_amount = paym_amount + move_line.debit - move_line.credit

                DettaglioPagamento = DettaglioPagamentoType(
                    ModalitaPagamento=(
                        invoice.payment_term.fatturapa_pm_id.code),
                    DataScadenzaPagamento=mat_date,
                    ImportoPagamento='%.2f' % paym_amount
                    )              
