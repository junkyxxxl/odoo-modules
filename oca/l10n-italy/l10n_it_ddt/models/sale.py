        ...
        for order in self.browse(cr, uid, ids, context):
            for picking in order.picking_ids:
                ...
                if order.transportation_method_id:
                    vals.update({'delivery_methods':order.transportation_method_id.id})
                self.pool.get('stock.picking').write(cr, uid, [picking.id], vals, context=context)                
            if order.create_ddt:
                ddt_data = {'partner_id': order.partner_id.id,}
                ddt_pool = self.pool['stock.ddt']
                ddt_id = ddt_pool.create(cr, uid, ddt_data, context=context)
                for picking in order.picking_ids:
                    self.pool.get('stock.picking').write(cr, uid, [picking.id], {'ddt_id': ddt_id})
                    picking.ddt_id = ddt_id
                workflow.trg_validate(uid, 'stock.ddt', ddt_id, 'ddt_confirm', cr)
        return res
