...
class CommonReportHeaderWebkit(common_report_header):
    ...
    def sort_accounts_with_structure(self, root_account_ids, account_ids,
        ...
        def recursive_sort_by_code(accounts, parent):
            ...
            level_accounts = sorted(level_accounts, key=lambda a: a['name'])
    ...
    def _compute_init_balance(self, account_id=None, period_ids=None,
                              mode='computed', default_values=False, target_move='all'):
        ...
        if not default_values:
            ...
            try:
                if target_move == 'all':
                    ...
                    self.cursor.execute("SELECT sum(lin.debit) AS debit, "
                                        " sum(lin.credit) AS credit, "
                                        " sum(lin.debit)-sum(lin.credit) AS balance, "
                                        " sum(lin.amount_currency) AS curr_balance"
                                        " FROM account_move_line AS lin, account_move AS mov"
                                        " WHERE lin.period_id in %s"
                                        " AND lin.account_id = %s"
                                        " AND lin.move_id = mov.id"
                                        " AND mov.state like 'posted'",
                                        (tuple(period_ids), account_id))                    
    ...
    def _read_opening_balance(self, account_ids, start_period, target_move='all'):
        ...
        for account_id in account_ids:
            res[account_id] = self._compute_init_balance(
                account_id, opening_period_selected, mode='read', target_move=target_move)
        return res

    def _compute_initial_balances(self, account_ids, start_period, fiscalyear, target_move='all'):
        ...
        for acc in self.pool.get('account.account').browse(self.cursor,
                                                           self.uid,
                                                           account_ids):
            res[acc.id] = self._compute_init_balance(default_values=True, target_move=target_move)
            if acc.user_type.close_method == 'none':
                ...
                res[acc.id] = self._compute_init_balance(acc.id, bs_period_ids, target_move=target_move)
        return res
    ...
    def _get_move_line_datas(self, move_line_ids,
                             order='per.special DESC, m.date ASC, \
