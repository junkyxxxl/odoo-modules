...
class PartnersOpenInvoicesWebkit(report_sxw.rml_parse,
    ...
    def set_context(self, objects, data, ids, report_type=None):
        ...
        date_maturity = self._get_form_param('maturity_date', data)
        ...
        for account_id in account_ids:
            acc_data = acc_obj.browse(self.cursor, self.uid, account_id)
            if acc_data and acc_data.parent_id and acc_data.parent_id.id in parent:
                tmp_account_ids.append(account_id)
        account_ids = tmp_account_ids
        ...
        ledger_lines_memoizer = self._compute_open_transactions_lines(
            ...
            date_maturity,partner_filter=partner_ids)
    ...
    def _check_account_partner_validity(self, account_id, partner_filter):
        if not partner_filter: 
            return True
        partner_obj = self.pool.get('res.partner')
        for partner_id in partner_filter:
            partner = partner_obj.browse(self.cr, self.uid, partner_id)
            if partner and partner.property_account_receivable and partner.property_account_receivable.id == account_id:
                return True
            if partner and partner.property_account_payable and partner.property_account_payable.id == account_id:
                return True
        return False

    def _compute_open_transactions_lines(self, accounts_ids, main_filter,
                                         target_move, start, stop,
                                         date_until=False,date_maturity=False,
        ...
        for account_id in accounts_ids:
            if not self._check_account_partner_validity(account_id, partner_filter):
                continue
            ...
            for partner_id in list(
                ...
                for line in lines:
                    ...
                    if date_maturity and line['date_maturity'] > date_maturity:
