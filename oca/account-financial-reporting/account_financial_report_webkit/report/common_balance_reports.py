...
class CommonBalanceReportHeaderWebkit(CommonReportHeaderWebkit):
    ...
    def _get_account_details(self, account_ids, target_move, fiscalyear,
        ...
        if initial_balance_mode == 'opening_balance':
            init_balance = self._read_opening_balance(account_ids, start, target_move=target_move)
        elif initial_balance_mode:
            init_balance = self._compute_initial_balances(
                account_ids, start, fiscalyear, target_move=target_move)
    ...
    def compute_balance_data(self, data, filter_report_type=None):
        ...
        if 'group_partner' in data['form'] and data['form']['group_partner']:
            ...
            account_to_remove_ids = self.pool.get('account.account').search(self.cr, self.uid, [('id','in',account_ids),('parent_id','in',parent)])

            if account_to_remove_ids:
               new_account_ids = []
               for account_id in account_ids:
                   if account_id not in account_to_remove_ids:
