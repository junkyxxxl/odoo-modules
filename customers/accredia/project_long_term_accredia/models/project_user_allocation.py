...
class ProjectUserAllocation(models.Model):
    ...
    phase_id = fields.Many2one('project.phase', 'Project Phase', ondelete='cascade')
    project_id = fields.Many2one(related='phase_id.project_id',
                                 comodel_name="project.project",
                                 string='Project',
                                 store=True)
    date_start = fields.Date('Start Date', help="Starting Date")
    date_end = fields.Date('End Date', help="Ending Date")
    num_days = fields.Float('Giorni')
