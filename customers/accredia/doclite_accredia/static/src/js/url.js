openerp.doclite_accredia = function(instance, m) {
    ...
    instance.web.Sidebar.include({
        redraw: function() {
            ...
            // nasconde pulsante Aggiungi (attachment)
            // self.$el.find('.oe_sidebar_add_attachment').hide();
        ...
        on_upload_doclite: function() {
            ...
            dataset.read_slice(['ip_address', 'port', 'base_path', 'server_location']).done(function(result) {
                _.each(result, function(v, k) {
                    var user = new instance.web.DataSetSearch(self, 'res.users', self.session.user_context, [
                        ['id', '=', self.session.uid]
                    ]);

                    return user.read_slice(['server_location']).then(function (res) {
                        if (_.isEmpty(res) )
                            return;
                        if (res[0].server_location === false){
                            return;
                        }
                        if (res[0].server_location === v.server_location){
