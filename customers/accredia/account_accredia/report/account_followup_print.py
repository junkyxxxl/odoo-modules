...
class report_rappel2(report_sxw.rml_parse):
    _name = "account_followup.report.rappel"

    def __init__(self, cr, uid, name, context=None):
        super(report_rappel2, self).__init__(cr, uid, name, context=context)
    ...
    def _lines_get(self, stat_by_partner_line, date):
        department_id = None
        if self.localcontext.get('default_dom_department_id'):
            department_id = self.localcontext.get('default_dom_department_id')
        return self._lines_get_with_partner(stat_by_partner_line.partner_id, stat_by_partner_line.company_id.id, date, department_id)

    def _lines_get_with_partner(self, partner, company_id, date, department_id):
        pool = pooler.get_pool(self.cr.dbname)
        ...
        for line in moveline_obj.browse(self.cr, self.uid, moveline_ids):
            ...
            if not line.blocked:
                ...
                pool.get('account.move.line').write(self.cr, self.uid, [line.id],
                                                         {'blocked': True})
...
report_sxw.report_sxw('report.account_followup.followup.print2',
                      ...
                      os.path.dirname(os.path.realpath(__file__)) + 
                                      '/account_followup_print.rml',
                      parser=report_rappel2)
