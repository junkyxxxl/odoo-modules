...
class report_ddt_price_return_parser(report_sxw.rml_parse):
    ...
    def __init__(self, cr, uid, name, context=None):
        ...
        if context is None:
            context = {}
        super(report_ddt_price_return_parser,
    ...
    def _get_tot_pcs(self, doc):
        ...
        for line in doc.ddt_lines_return:
            res -= line.product_uom_qty            
        return int(res)
    ...
    def _get_records(self, doc):
        ...
        for prod_id in self._get_templates(doc, False):
            if not prod_id.is_shipping:        
                tmp = []
                tmp.append(False)
                tmp.append(prod_id.tmpl_default_code)
                tmp.append('')
                tmp.append(prod_id.name)
                for i in range(0,10):
                        tmp.append('')

                inv_line = self._get_line_no_dimensions(doc, prod_id)
                if inv_line:
                    tmp.append(int(inv_line))
                else:
                    tmp.append('')  
    ...
    def _get_totals(self, doc):
        ...
        for ddt_line in doc.ddt_lines_return:
            if ddt_line.procurement_id and ddt_line.procurement_id.sale_line_id:
                line = ddt_line.procurement_id.sale_line_id
                if line.free not in ['gift', 'base_gift']:
                    original_tax_value = 0.0
    ...
    def _get_line(self, doc, template, color, size):
        ...
        returns = []
        for picking in doc.picking_ids_return:
    ...
    def _get_line_no_dimensions(self, doc, product):
        res = 0.0
        for line in doc.ddt_lines:
...
class report_ddt_price_return(osv.AbstractModel):
    _name = 'report.report_qweb_montecristo.report_ddt_price_return'
    _inherit = 'report.abstract_report'
    _template = 'report_qweb_montecristo.report_ddt_price_return'
    _wrapped_report_class = report_ddt_price_return_parser
