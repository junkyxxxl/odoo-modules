...
class wzd_invoice_commissions_payment(orm.TransientModel):
    ...
    _columns = {
        ...
        'salesagent_id': fields.many2one('res.partner','Salesagent', required = True)        
        }
    ...
    def pagamento_provvigioni(self, cr, uid, ids, context={}):
        ...
        for invoice in invoices:
            for line in invoice.invoice_line:
                ...
                if not wizard_obj.salesagent_id.id or  line.salesagent_id.id == wizard_obj.salesagent_id.id:
                ...
                if not wizard_obj.salesagent_id.id or  line.salesagent_id_base.id == wizard_obj.salesagent_id.id:
                    line_obj.write(cr, uid, [line.id,], {
                        'payment_commission_date_base' : wizard_obj.payment_date,
                        'paid_commission_base' : True,
                        'paid_commission_value_base' : line.commission,
                        'paid_commission_percentage_value_base' : line.commission_percentage,
                        'payment_commission_note_base':wizard_obj.payment_commission_note})  
