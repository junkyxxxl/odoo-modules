...
class riba_unsolved(orm.TransientModel):
    ...
    def _get_overdue_effects_account_id(self, cr, uid, context=None):
        ...
        if not context.get('active_id', False):
            return False        
        return self.pool.get('riba.distinta.line').browse( cr, uid, context['active_id'], context=context).partner_account_id
    
    def _get_bank_account_id(self, cr, uid, context=None):
        return self.pool.get('riba.configurazione').get_default_value_by_distinta_line( cr, uid, 'unsolved_bank_account_id', context=context)
    ...
    _columns = {
        ...
        'payment_type': fields.selection([('C', 'Contante'),
                                          ...
                                          ('D', 'Ricevuta Bancaria')],
                                         'Tipo Pagamento',),
        'date_maturity': fields.date('Scadenza'),
        }
    ...
    _defaults = {
        ...
        'bank_amount': _get_effects_amount,
        ...
        if wizard.overdue_effects_amount:
            move_vals['line_id'].append(
                    (0,0, {
                        ...
                        'payment_type_move_line': wizard.payment_type,
                        })                                    
            )
