...
class emissione_riba(orm.TransientModel):
    ...
    def crea_distinta(self, cr, uid, ids, context=None):
        ...
        rd_id = riba_distinta.create(cr, uid, rd,context=context)
...
#                        invoice_date_group = ''
                        for grouped_line in grouped_lines[key]:
                            ...
                            if grouped_line.debit > 0:
                                vals.update({'amount': grouped_line.amount_residual})
                            else:
                                vals.update({'amount': -grouped_line.amount_residual})     
                                                           
                            riba_distinta_move_line.create(cr, uid, vals, context=context)
                ...
                if move_line.debit > 0:
                    vals.update({'amount': move_line.amount_residual,})
                else:
                    vals.update({'amount': -move_line.amount_residual,})     
