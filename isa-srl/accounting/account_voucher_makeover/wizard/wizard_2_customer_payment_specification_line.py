...
class wizard_customer_payment_specification_line(orm.TransientModel):
    _name = 'wizard.customer.payment.specification.line'
    _description = 'Wizard Customer Payment Specification Line'
    ...
    _columns = {
        ...
        'payment_specification_id': fields.many2one('wizard.customer.payment.specification',
    ...
    def set_payment_lines(self, cr, uid, context=None):
        ...
        wizard_obj = self.pool.get('wizard.customer.payment.specification')
        wizard_cus_obj = self.pool.get('wizard.customer.payment')
        
        t_lines = []
        t_all_customer = False
        list_partner = []
        t_limit = 50
        ...
        if(t_partner_id):
            ...
            s_filters = wizard_obj.get_wizard_customer_filters(cr, uid, context)
        ...
        t_total_pages = wizard_cus_obj.get_total_pages(cr, uid, v_filters, t_limit)
        ...
        for t_move_line in account_move_line_obj.browse(cr, uid, account_move_line_valid_ids):
            ...
            t_is_out_invoice = wizard_obj.check_move_line_out_invoice(cr, uid, [t_move_line.id], context)
            if(t_is_out_invoice):
                t_lines.append((0, 0, {
    ...
    def move_draft(self, fb, cr, uid, ids, context=None):
        ...
        t_all_customer = data.payment_specification_id.all_customer
        t_it_customer = data.payment_specification_id.it_customer
        t_ext_customer = data.payment_specification_id.ext_customer
        t_riba = data.payment_specification_id.riba
