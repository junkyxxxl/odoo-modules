...
class wizard_customer_set_partial_amount(orm.TransientModel):
    _name = 'wizard.customer.set.partial.amount'
    _description = 'Wizard Set Partial Amount'

    _columns = {
              'line_id': fields.many2one('wizard.confirm.customer.payment.line', 'Line'),
    ...
    def onchange_amount_partial(self, cr, uid, ids, amount_initial, amount_partial, context=None):
        t_residual = amount_partial - amount_initial
    ...
    def confirm(self, cr, uid, ids, context=None):
        ...
        t_allowance_residual = form["amount_partial"] - form["amount_initial"]
                
        t_obj = self.pool.get('wizard.confirm.customer.payment.line')
        ...
        if(t_check_value < t_amount_partial and t_allowance):
            raise orm.except_orm(_('Error!'), _('Per abbuono attivo inserire importo inferiore di ' + str(t_check_value)))
        if(t_check_value_2 > t_amount_partial and t_allowance):
            raise orm.except_orm(_('Error!'), _('Per abbuono passivo inserire importo maggiore di ' + str(t_check_value_2)))
