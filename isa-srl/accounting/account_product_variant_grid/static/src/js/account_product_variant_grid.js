openerp.account_product_variant_grid = function(instance) {
    ...
    instance.account_product_variant_grid.qtyWidget = instance.web.form.FormWidget.extend(instance.web.form.ReinitializeWidgetMixin, {
 ...
 //           window.alert("init");
	    ...
	    self.$('[name="annulla"]').attr( "disabled", true );		
            ...
            this.updating = false;
        	this.defs = [];
            this.field_manager.on("field_changed:invoice_line", this, this.query_quants);
	    ...
	    this.field_manager.on("field_changed:show_totals", this, function() 
            ...
            this.field_manager.on("field_changed:journal_id", this, function() 
            {
                this.set({"journal_id": this.field_manager.get_field_value("journal_id")});
            });
            this.field_manager.on("field_changed:state", this, function() 
            {
                this.set({"state": this.field_manager.get_field_value("state")});
            });
   //         this.on("change:invoice_lines", this, this.update_quants);
  ...
  //          window.alert("query_quants");
            ...
            if (self.updating)
                return;
            var commands = this.field_manager.get_field_value("invoice_line");

            this.res_o2m_drop.add(new instance.web.Model(this.view.model).call("fnct_prova", ["invoice_line", commands, [], 
                    new instance.web.CompoundContext()]))
                .done(function(result) {
                self.querying = true;
                self.field_manager.set_values({template_id: false}); 
                self.field_manager.set_values({value_filter_id: false});		
                self.set({invoice_lines: result});
                self.querying = false;
            });
...
//          window.alert("update_quants");
    		...
    		if(tmp[i]['state']=='sent')
		{
		  tmp[i]['state']='draft';
            ...
            self.field_manager.set_values({invoice_line: self.get("invoice_lines")}).done(function() {
                self.updating = false;
            });
 ...
 //           window.alert("initialize_field");
            ...
            var self = this;
            self.on("change:invoice_lines", self, self.initialize_content);
	...
	tmp_initialize_content: function()
	{
	    ...
	    var product_product = new instance.web.Model("product.product");                
            ...
            {	
            	...
            	self.$el.html(QWeb.render("account_product_variant_grid.qtyWidget", {widget: self}));	
            	return;           
            }
...
//            			self.update_quants();
            			xyz = self.field_manager.get_field_value("invoice_line");
            			...
            			for(i=0;i<xyz.length;i++)
            			{
            				if(xyz[i][2]!=false)
            				{
            					...
            					q_products[j][1]=xyz[i][2]["quantity"];
 ...
 //       	window.alert("display_data");
		...
		self.$('[name="annulla"]').click(function()
		{
            ...
            _.each(self.rows, function(row) {
				...
				{
					self.get_box(rows_name[_.indexOf(rows,row)], columns_name[_.indexOf(columns,column)]).attr( "class", "oe_account_invoice_grid_input_focused" );
				}
				...
				var num = $(this).val();
					...
					self.field_manager.set_values({saved: false});				  
	                        	...
	                        	{
	                        		$(this).val(0);	
						$(this).attr( "class", "oe_account_invoice_grid_input" );
	                        	}
	                        	else
					{
						$(this).attr( "class", "oe_account_invoice_grid_input_focused" );  
					}                        	
	                        	tmpRow = {
	                        	           'quantity':Num,
	                        	...
	                        	new instance.web.Model("account.invoice.line").call("product_id_change", [new Array(),tmpRow["product_id"], false, tmpRow["quantity"], false, self.field_manager.get_field_value("type"), self.field_manager.get_field_value("partner_id"), self.field_manager.get_field_value("fiscal_position"),0, self.field_manager.get_field_value("currency_id"), self.field_manager.get_field_value("company_id")]).then(function(result)
	                            {	
	                            	if("warning" in result && tmpRow["quantity"]!=0)
	                            	{
	                            	...
	                            	tmpRes=
	                            	{
	                     	           'quantity':tmpRow["quantity"],
	                     	           'product_id' :tmpRow["product_id"],
	                     	           'price_unit' : result["value"]["price_unit"],
	                     	           'invoice_line_tax_id' : [[6,false,result["value"]["invoice_line_tax_id"]]],
	                     	         //  'state' : self.get("state"),
	                     	           'name':result["value"]["name"],
	                     	           'uos_id':result["value"]["uos_id"],
	                     	           'account_id':result["value"]["account_id"],
					   'company_id':self.field_manager.get_field_value("company_id")
	                            	}                         	
	                            	self.sync(tmpRes);
					products_qty[_.indexOf(rows,row)][_.indexOf(columns,column)]=Num;
			  ...
			  if (self.products_qty[_.indexOf(rows,row)][_.indexOf(columns,column)] > 0)
			  {
			    self.get_box(rows_name[_.indexOf(rows,row)], columns_name[_.indexOf(columns,column)]).attr( "class", "oe_account_invoice_grid_box_focused" );
			  }
  ...
  //          window.alert("display_totals");
		...
		if (self.get("show_totals"))
		  self.get_row_total(rows_name[_.indexOf(rows,row)]).html(row_sum.toString()+'/'+row_sum_price.toFixed(2).toString()+'€'/*self.get('currency_id').symbol*/); /*MODIFICATA*/
		else
		  self.get_row_total(rows_name[_.indexOf(rows,row)]).html(row_sum.toString()); /*MODIFICATA*/		
		    ...
		    if (self.get("show_totals"))
		      self.get_column_total(columns_name[_.indexOf(columns,column)]).html(column_sum[_.indexOf(columns,column)].toString()+'/'+column_sum_price[_.indexOf(columns,column)].toFixed(2).toString()+'€'/*self.get('currency_id').symbol*/);		/*MODIFICATA*/
		    else
		      self.get_column_total(columns_name[_.indexOf(columns,column)]).html(column_sum[_.indexOf(columns,column)].toString()); /*MODIFICATA*/
                });
		if (self.get("show_totals"))		
		  self.get_super_total().html(super_tot.toString()+'/'+super_tot_price.toFixed(2).toString()+'€'/*self.get('currency_id').symbol*/);		/*MODIFICATA*/
		else
...
//  		window.alert("display_icons");
			...
			{
			  if(self.copied_row != null && self.copied_price != null && self.copied_row.length > 0 && self.copied_price.length > 0) 	/*MODIFICATA*/
			  {		    
			    for(i=0;i<columns.length;i++)
			    {
			    ...
			    {
				tmpRow = {
					    'quantity':self.products_qty[_.indexOf(rows,row)][_.indexOf(columns,column)],
					    'product_id' :products_id[_.indexOf(rows,row)][_.indexOf(columns,column)],
					    'state' : self.get("state")              	           
				};
...
//            window.alert("sync - "+JSON.stringify(record));
            var self = this;
	    var ops = self.get("invoice_lines");
            self.set({invoice_lines: this.generate_o2m_value(record)});	    
//            self.update_quants();
        },
	buffered_sync: function() {
//	    window.alert("buffered_sync");
	    var self = this;
	    self.record = self.get("invoice_lines");    
...
//            self.update_quants();
    ...
    instance.web.form.custom_widgets.add('account_invoice_grid_widget', 'instance.account_product_variant_grid.qtyWidget');
