openerp.sale_product_variant_grid = function(instance) {
    ...
    instance.sale_product_variant_grid.qtyWidget = instance.web.form.FormWidget.extend(instance.web.form.ReinitializeWidgetMixin, {
...
//          window.alert("init");
	    ...
	    this.field_manager.on("field_changed:show_uos", this, function() 
            {
                this.set({"show_uos": this.field_manager.get_field_value("show_uos")});
            });		    
	...
	//    this.on("change:template_id", this, this.update_quants);
...
//	    self.update_quants();
            ...
            {	
            	...
            	self.$el.html(QWeb.render("sale_product_variant_grid.qtyWidget", {widget: self}));
            	return;           
            }
			...
			self.products_uos_qty = products_uos_qty;	/***/
			self.copied_row = copied_row;			
			self.copied_price = copied_price;		/**/
			self.copied_uos = copied_uos;			/***/
...
//            			self.update_quants();
            			...
            			for(i=0;i<xyz.length;i++)
            			{
            				if(xyz[i][2]!=false)
            				{
            					...
            					q_products[j][1]=xyz[i][2]["product_uom_qty"];
						q_products[j][2]=xyz[i][2]["price_unit"];		/**/
						q_products[j][3]=xyz[i][2]["product_uos_qty"];		/***/
            					j++;
            				}           		
            			}
				...
				products_uos_qty = new Array();					/***/
					...
					products_uos_qty[0][index_c]=pr_uos;			/***/
                            	}
		                		self.columns = columns;
		                		self.columns_name = columns_name;
						self.columns_lenght = columns.length;
...
//        	window.alert("display_data");
		...
		self.$('[name="annulla"]').click(function()
		{
            ...
            _.each(self.rows, function(row) {
				...
				{
					self.get_box(rows_name[_.indexOf(rows,row)], columns_name[_.indexOf(columns,column)]).attr( "class", "oe_sale_order_grid_input_focused" );
				}
				...
				var num = $(this).val();
					...
					self.field_manager.set_values({saved: false});
	                        	...
	                        	{
	                        		$(this).val(0);
						$(this).attr( "class", "oe_sale_order_grid_input" );
	                        	}
	                        	else
					{
						$(this).attr( "class", "oe_sale_order_grid_input_focused" );  
					}
	                        	tmpRow = {
	                        	           'product_uom_qty':Num,
	                        	...
	                        	self.display_totals();	
	                        	new instance.web.Model("sale.order.line").call("product_id_change", [new Array(),self.field_manager.get_field_value("pricelist_id"), tmpRow["product_id"], tmpRow["product_uom_qty"], false, tmpRow["product_uom_qty"], false, "", self.field_manager.get_field_value("partner_id"), false, true, self.field_manager.get_field_value("date_order"), false, self.field_manager.get_field_value("fiscal_position"), false]).then(function(result)
	                            {	
	                      //  		window.alert(JSON.stringify(result));
	                      //  		window.alert(JSON.stringify(result["value"]));
				      /*
	                            	if("warning" in result && tmpRow["product_uom_qty"]!=0)
	                            	{
				      ...
				      */
	                            	tmpRes=
	                            	{
	                     	           'product_uos_qty':result["value"]["product_uos_qty"],
	                     	           ...
	                     	           'tax_id' : [[6,false,result["value"]["tax_id"]]],
	                     	           'state' : self.get("state"),
	                     	           'th_weight':result["value"]["th_weight"],
	                     	           'product_uom':result["value"]["product_uom"],
	                     	           'name':result["value"]["name"],
	                     	           'delay':result["value"]["delay"],
	                     	           'product_uos':result["value"]["product_uos"],
	                     	           'product_packaging':result["value"]["product_packaging"]
	                            	}
					...
					//Controllo necessario per cliente Montecristo
	                            	if(typeof(result["value"]["salesagent"])!=undefined)
					{
					  tmpRes['salesagent'] = result["value"]["salesagent"];
					}
					//Controllo necessario nel caso in cui sia installato il modulo 'sale_margin'
	                            	if(typeof(result["value"]["purchase_price"])!=undefined)
					{
					  tmpRes['purchase_price'] = result["value"]["purchase_price"];
					}					
					//Controlli necessari per cliente Mattioli
	                            	if(typeof(result["value"]["price_uos"])!=undefined)
					{
					  tmpRes['price_uos'] = result["value"]["price_uos"];
					}					
	                            	if(typeof(result["value"]["length"])!=undefined)
					{
					  tmpRes['length'] = result["value"]["length"];
					}
	                            	if(typeof(result["value"]["width"])!=undefined)
					{
					  tmpRes['width'] = result["value"]["width"];
					}
	                            	if(typeof(result["value"]["thickness"])!=undefined)
					{
					  tmpRes['thickness'] = result["value"]["thickness"];
					}					
					
	                            	self.sync(tmpRes);
					products_qty[_.indexOf(rows,row)][_.indexOf(columns,column)]=Num;
					products_price[_.indexOf(rows,row)][_.indexOf(columns,column)]=tmpRes['price_unit'];			/**/
					products_uos_qty[_.indexOf(rows,row)][_.indexOf(columns,column)]=tmpRes['product_uos_qty'];		/***/
	                        	self.display_totals();
					self.lock = false;
	                            });
	                        }
			  ...
			  if (self.products_qty[_.indexOf(rows,row)][_.indexOf(columns,column)] > 0)
			  {
			    self.get_box(rows_name[_.indexOf(rows,row)], columns_name[_.indexOf(columns,column)]).attr( "class", "oe_sale_order_grid_box_focused" );
			  }
  ...
  //          window.alert("display_totals");
		...
		var column_sum_uos = _.map(_.range(self.columns.length), function() { return 0}); /***/
        	var super_tot = 0;
		...
		var super_tot_uos = 0;			/***/
			...
			var sum_uos = self.products_uos_qty[_.indexOf(rows,row)][_.indexOf(columns,column)];	/***/
			
                	super_tot+=sum;
			super_tot_price+=sum_price*sum;		/**/
			super_tot_uos+=sum_uos*sum;		/***/
                	row_sum+=sum;
			row_sum_price+=sum_price*sum;		/**/
			row_sum_uos+=sum_uos*sum;		/***/
                	column_sum[_.indexOf(columns,column)]+=sum;
			column_sum_price[_.indexOf(columns,column)]+=sum_price*sum;			/**/
			column_sum_uos[_.indexOf(columns,column)]+=sum_uos*sum;				/***/
                });
		
		if (self.get("show_totals"))
		  if (self.get("show_uos"))
		    self.get_row_total(rows_name[_.indexOf(rows,row)]).html(row_sum.toString()+'/'+row_sum_uos.toFixed(4).toString()+'/'+row_sum_price.toFixed(2).toString()+'€'); /*MODIFICATA*/		    
		  else
		    self.get_row_total(rows_name[_.indexOf(rows,row)]).html(row_sum.toString()+'/'+row_sum_price.toFixed(2).toString()+'€'); /*MODIFICATA*/
		else
		  if (self.get("show_uos"))
		    self.get_row_total(rows_name[_.indexOf(rows,row)]).html(row_sum.toString()+'/'+row_sum_uos.toFixed(4).toString()); /*MODIFICATA*/				    
		  else
		    ...
		    if (self.get("show_totals"))
		      if (self.get("show_uos"))
			self.get_column_total(columns_name[_.indexOf(columns,column)]).html(column_sum[_.indexOf(columns,column)].toString()+'/'+column_sum_uos[_.indexOf(columns,column)].toFixed(4).toString()+'/'+column_sum_price[_.indexOf(columns,column)].toFixed(2).toString()+'€');		/*MODIFICATA*/		      
		      else
			self.get_column_total(columns_name[_.indexOf(columns,column)]).html(column_sum[_.indexOf(columns,column)].toString()+'/'+column_sum_price[_.indexOf(columns,column)].toFixed(2).toString()+'€');		/*MODIFICATA*/
		      ...
		      if (self.get("show_uos"))
			self.get_column_total(columns_name[_.indexOf(columns,column)]).html(column_sum[_.indexOf(columns,column)].toString()+'/'+column_sum_uos[_.indexOf(columns,column)].toFixed(4).toString()); /*MODIFICATA*/		      
		      else
			self.get_column_total(columns_name[_.indexOf(columns,column)]).html(column_sum[_.indexOf(columns,column)].toString()); /*MODIFICATA*/
                });
		if (self.get("show_totals"))
		  if (self.get("show_uos"))
		    self.get_super_total().html(super_tot.toString()+'/'+super_tot_uos.toFixed(4).toString()+'/'+super_tot_price.toFixed(2).toString()+'€');		/*MODIFICATA*/
		  else
		    self.get_super_total().html(super_tot.toString()+'/'+super_tot_price.toFixed(2).toString()+'€');		/*MODIFICATA*/
		else
		  if (self.get("show_uos"))
		    self.get_super_total().html(super_tot.toString()+'/'+super_tot_uos.toFixed(4).toString());		/*MODIFICATA*/
		  else
...
//  		window.alert("display_icons");
			...
			self.get_copy(rows_name[_.indexOf(rows,row)]).click(function() 
			{
				...
				self.copied_uos[i]=self.products_uos_qty[_.indexOf(rows,row)][i];			/***/
			    }
			});
			self.get_paste(rows_name[_.indexOf(rows,row)]).click(function() 
			{
			  if(self.copied_row != null && self.copied_price != null && self.copied_row.length > 0 && self.copied_uos != null &&  self.copied_price.length > 0 && self.copied_uos.length > 0) 	/*MODIFICATA*/
			  {
			    for(i=0;i<columns.length;i++)
			    {
				self.products_qty[_.indexOf(rows,row)][i]=self.copied_row[i];				
				self.products_price[_.indexOf(rows,row)][i]=self.copied_price[i];			/**/
				self.products_uos_qty[_.indexOf(rows,row)][i]=self.copied_uos[i];			/***/
			    }
//			    window.alert('Incolla:'+JSON.stringify(self.products_price[_.indexOf(rows,row)]));
			    _.each(self.columns, function(column) 
			    {
				tmpRow = {
					    'product_uom_qty':self.products_qty[_.indexOf(rows,row)][_.indexOf(columns,column)],
					    'product_id' :products_id[_.indexOf(rows,row)][_.indexOf(columns,column)],
					    'state' : self.get("state")              	           
				};
...
//	      window.alert("record: "+JSON.stringify(self.to_sync[t]));
		...
		else 
		{
	      ...
	      if(self.to_sync[t].product_uom_qty>0)
...
//            window.alert("generate_o2m_value");
    ...
    instance.web.form.custom_widgets.add('sale_grid_widget', 'instance.sale_product_variant_grid.qtyWidget');
