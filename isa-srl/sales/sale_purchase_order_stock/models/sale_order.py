...
class sale_order_shipping_state(osv.osv):
    _name = "sale.order.shipping.state"
    
    _columns = {
                'order_id':fields.many2one('sale.order', string="Ordine", required=True, ondelete='cascade'),
                ...
                'shipped_qty':fields.float('Quantità Trasferita', digits_compute=dp.get_precision('Product Unit of Measure')),
...
class sale_order_shipping_state(osv.osv):
    ...
    _columns = {
                'shipping_state_ids': fields.one2many('sale.order.shipping.state','order_id','Stato Trasferimento Ordine'), 
    }

    def compute_shipped_quantities(self, cr, uid, ids, context=None):
        order_id = self.browse(cr,uid,ids[0])

        line_obj = self.pool.get('sale.order.shipping.state')
        ...
        for so in self.browse(cr, uid, ids, context=context):
            for line in so.order_line:
                if line.product_id:
                    ...
                    to_create[line.product_id.id][0] += line.product_uom_qty
