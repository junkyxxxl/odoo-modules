...
class wizard_confirm_payment_line(orm.TransientModel):

    _name = 'wizard.confirm.payment.line'
    _description = 'Wizard Confirm Payment Line'
    ...
    _columns = {
        ...
        'confirm_payment_id': fields.many2one('wizard.confirm.payment',
                                         'Confirm Payment',
                                         ondelete="cascade",
                                         required=True),
    ...
    def set_partial_amount(self, cr, uid, ids, context=None):
        ...
        result = mod_obj.get_object_reference(cr, uid,
                                              ...
                                              'wizard_set_partial_amount_account_voucher_makeover_view')
        ...
        return {
              ...
              'view_type': 'form',
              'view_mode': 'form',
              'res_model': 'wizard.set.partial.amount',
    ...
    def delete_payment(self, cr, uid, ids, context=None):
        ...
        t_all_supplier = data.confirm_payment_id.all_supplier
        t_it_supplier = data.confirm_payment_id.it_supplier
        t_ext_supplier = data.confirm_payment_id.ext_supplier
    ...
    def set_confirm_payment_lines(self, cr, uid, context=None):
        ...
        t_it_suppliers = context.get('default_it_supplier', None)
        t_ext_suppliers = context.get('default_ext_supplier', None)
        ...
        if(t_all_suppliers):
            ...
            if(t_ext_suppliers):
                ...
                f = ('country_id', '!=', t_italy_id)
                supplier_filter.append(f)
            if(len(supplier_filter) > 1):
                supplier_filter.insert(0, '&')
            list_partner = []
            vals['partner_id'] = None
            list_partner = self.pool.get('res.partner').search(cr, uid, supplier_filter)
